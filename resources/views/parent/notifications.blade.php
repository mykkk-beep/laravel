...
<div class="mx-auto w-full max-w-5xl px-3 py-4 sm:px-4 sm:py-6 md:px-6">
    ...
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <h1 class="text-xl font-bold tracking-tight text-slate-900 sm:text-2xl md:text-3xl">Messages for {{ $student->name }}</h1>
            <p class="mt-2 text-xs sm:text-sm text-slate-500">Review and reply to teacher messages</p>
        </div>
        <a href="{{ route('parent.dashboard') }}" class="inline-flex min-h-10 w-full items-center justify-center rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs sm:text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50 active:scale-95 sm:w-auto">
            ← Back to dashboard
        </a>
    </div>
        ...
        @forelse($notifications as $notification)
            <div class="card overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                ...
                <div class="border-b border-slate-100 bg-gradient-to-r from-slate-50 to-indigo-50 px-4 py-3 sm:px-6 sm:py-4">
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <div class="flex flex-wrap items-center gap-2 sm:shrink-0">
                            ...
                            <form method="POST" action="{{ route('parent.notifications.destroy', $notification) }}" class="inline-block">
                                @csrf
                                @method('DELETE')
                ...
                <div class="p-4 sm:p-6">
                        ...
                        @foreach($notification->replies as $reply)
                            @if($reply->sender === 'parent')
                                ...
                                <div class="flex gap-3 justify-end">
                                    ...
                                    <div class="flex-shrink-0">
                                        <div class="flex h-8 w-8 sm:h-10 sm:w-10 items-center justify-center rounded-full bg-emerald-600 text-xs sm:text-sm font-bold text-white">P</div>
                                    </div>
                                </div>
                            @else
                    ...
                    <!-- Reply Form -->
                    <form method="POST" action="{{ route('parent.notifications.reply', $notification) }}" class="border-t border-slate-200 pt-4">
                        @csrf
